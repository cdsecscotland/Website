...
export default function Accreditations() {
  return (
    <section className="py-12 bg-white/85 dark:bg-white/10 backdrop-blur-sm border-b border-gray-100/50 dark:border-gray-800/50">
      <div className="container">
        <ScrollAnimation animation="fade">
          <div className="text-center mb-8">
            <div className="flex items-center justify-center mb-4">
              <Shield className="w-8 h-8 text-brandyellow mr-3" />
              <h2 className="text-3xl md:text-4xl font-bold text-charcoal dark:text-white">Trusted & Accredited</h2>
            </div>
            <p className="text-lg text-charcoal/80 dark:text-white/80 max-w-3xl mx-auto">
              ...
              {certificationBadges.map((cert, index) => (
                <ScrollAnimation key={index} animation="fade" delay={0.4 + index * 0.05}>
                  <div className="flex items-start p-4 bg-white/70 dark:bg-white/10 backdrop-blur-sm rounded-lg hover:shadow-md transition-all duration-300 border border-white/40 dark:border-white/20">
                    ...
                    <div>
                      <h5 className="font-semibold text-charcoal dark:text-white text-sm mb-1">{cert.name}</h5>
                      <p className="text-xs text-charcoal/60 dark:text-white/60 mb-2">{cert.description}</p>
                      <Badge variant="secondary" className="text-xs bg-brandyellow/20 text-charcoal dark:text-white">
                        {cert.category}
                      </Badge>
                    </div>
                  </div>
