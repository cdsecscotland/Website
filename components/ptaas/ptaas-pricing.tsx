            ...
            <ScrollAnimation key={index} animation="slide-up" delay={0.2 + index * 0.1}>
              <Card
                className={`relative hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border-0 shadow-lg h-full ${
                  plan.popular
                    ? "bg-gradient-to-b from-brandyellow/10 to-brightyellow/10 dark:from-brandyellow/5 dark:to-brightyellow/5 ring-2 ring-brandyellow"
                    : "bg-white dark:bg-charcoal/80"
                }`}
              >
                {plan.popular && (
                ...
                <CardHeader className="text-center pb-4">
                  <CardTitle className="text-2xl text-charcoal dark:text-white">{plan.name}</CardTitle>
                  <div className="mt-4">
                    <span className="text-4xl font-bold text-charcoal dark:text-white">{plan.price}</span>
                    <span className="text-charcoal/60 dark:text-white/60 ml-2">/{plan.period}</span>
                  </div>
                  <p className="text-charcoal/70 dark:text-white/70 mt-2">{plan.description}</p>
                </CardHeader>
                ...
                <CardContent>
                  <ul className="space-y-3 mb-8">
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start">
                        ...
                        <span className="text-charcoal/80 dark:text-white/80">{feature}</span>
                      </li>
                    ))}
                  </ul>
                  ...
                  <Button
                    className={`w-full ${
                      plan.popular
                        ? "bg-charcoal dark:bg-brandyellow hover:bg-charcoal/90 dark:hover:bg-brightyellow text-white dark:text-charcoal"
                        : "bg-transparent border-2 border-charcoal dark:border-white text-charcoal dark:text-white hover:bg-charcoal hover:text-white dark:hover:bg-white dark:hover:text-charcoal"
                    }`}
                    size="lg"
                  >
                    {plan.cta}
